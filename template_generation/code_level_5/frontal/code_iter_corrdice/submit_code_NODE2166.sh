#!/bin/bash -l
if  QID0_1=$(sbatch -p cpu /HPC_work_dir/code_iter_corrdice/queue_NODE2166_0.sh 2>&1); then
...
if  QID0_2=$(sbatch -p cpu -d $QID0_1_id /HPC_work_dir/code_iter_corrdice/queue_mergeNODE2166_0.sh 2>&1); then
...
if  QID1_1=$(sbatch -p cpu -d $QID0_2_id /HPC_work_dir/code_iter_corrdice/queue_NODE2166_1.sh 2>&1); then
...
if  QID1_2=$(sbatch -p cpu -d $QID1_1_id /HPC_work_dir/code_iter_corrdice/queue_mergeNODE2166_1.sh 2>&1); then
...
while [ `echo "$diff > $thre" | bc` -eq 1 ]
    ...
    if  QID1_1=$(sbatch -p cpu /HPC_work_dir/code_iter_corrdice/queue_NODE2166_${iter}.sh 2>&1); then
        QID1_1_id=$(echo ${QID1_1} | sed -n -e 's/^.*job //p')
        echo "Job submitted successfully with ID $QID1_1_id"
    else
        echo "Job submission failed"
    ...
    if  QID1_2=$(sbatch -p cpu -d $QID1_1_id /HPC_work_dir/code_iter_corrdice/queue_mergeNODE2166_${iter}.sh 2>&1); then
        QID1_2_id=$(echo ${QID1_2} | sed -n -e 's/^.*job //p')
        echo "Job submitted successfully with ID $QID1_2_id"
    else
        echo "Job submission failed"
